...
IcGlissando::IcGlissando(Fluid* f, const char* filename, double _tau0, const char* setup) {
 ...
 } else { //reading parameters from file setup
  ifstream file_setup(setup);
  if (!file_setup.is_open()) {
   cout << "IcGlissando: optional parameter LHC276 or RHIC200 is expected and no file to open\n";
   exit(0);
  }
  char parName[255], parValue[255];
  while (file_setup.good()) {
  string line;
  getline(file_setup, line);
...
cout << "Custom setup Glissando from "<< setup<<endl;
