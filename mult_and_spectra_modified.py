...
class BulkObservables:
    ...
    def add_block(self, block):
        ...
        for i in range(self.npdg):
            ...
            self.sinrap[i,:] += np.histogram(y[pdgcut], bins = self.ybins,weights = sinphi[pdg_and_y_cut])[0]
            self.cosrap[i,:] += np.histogram(y[pdgcut], bins = self.ybins,weights = cosphi[pdg_and_y_cut])[0]
    ...
    def save(self, files_to_write):
        ...
        with open(cos2phi_file, 'w') as f:
             ...
             for bin_number in range(ptbin_centers.size):
                 ...
                 for i in range(self.npdg):
                     # For each pT, sum of cos2phi over particles has to be divided by number of particles at this pT
                     v2 = np.where(self.pthist_midrapidity[i, bin_number] > 0,
                                   self.cos2phi[i, bin_number] / self.pthist_midrapidity[i, bin_number], 0.0)
                     f.write(' %15.10f' % v2)
                 f.write('\n')
        ...
        with open(sin2phi_file, 'w') as f:
             ...
             for bin_number in range(ptbin_centers.size):
                 ...
                 for i in range(self.npdg):
                     # For each pT, sum of cos2phi over particles has to be divided by number of particles at this pT
                     v2 = np.where(self.pthist_midrapidity[i, bin_number] > 0,
                                   self.sin2phi[i, bin_number] / self.pthist_midrapidity[i, bin_number], 0.0)
                     f.write(' %15.10f' % v2)
                 f.write('\n')
        with open(sinphi_file, 'w') as f:
             ...
             for bin_number in range(ptbin_centers.size):
                 ...
                 for i in range(self.npdg):
                     # For each pT, sum of cos2phi over particles has to be divided by number of particles at this pT
                     v2 = np.where(self.pthist_midrapidity[i, bin_number] > 0,
                                   self.sinphi[i, bin_number] / self.pthist_midrapidity[i, bin_number], 0.0)
                     f.write(' %15.10f' % v2)
                 f.write('\n')
        ...
        with open(cosphi_file, 'w') as f:
             ...
             for bin_number in range(ptbin_centers.size):
                 ...
                 for i in range(self.npdg):
                     # For each pT, sum of cos2phi over particles has to be divided by number of particles at this pT
                     v2 = np.where(self.pthist_midrapidity[i, bin_number] > 0,
                                   self.cosphi[i, bin_number] / self.pthist_midrapidity[i, bin_number], 0.0)
                     f.write(' %15.10f' % v2)
                 f.write('\n')
        ...
        with open(sinrap_file, 'w') as f:
            ...
            for bin_number in range(ybin_centers.size):
                f.write(' %6.3f' % ybin_centers[bin_number])
                for i in range(self.npdg):
                     f.write(' %8i' % self.sinrap[i, bin_number])
                f.write('\n')
        ...
        with open(cosrap_file, 'w') as f:
            ...
            for bin_number in range(ybin_centers.size):
                f.write(' %6.3f' % ybin_centers[bin_number])
                for i in range(self.npdg):
                     f.write(' %8i' % self.cosrap[i, bin_number])
                f.write('\n')
    ...
    def read(files_to_read):
        ...
        cos2phi_file,sin2phi_file,cosphi_file,sinphi_file= files_to_read
...
if __name__ == '__main__':
    ...
    if (args.output_files_extended):
        output_files=["yspectra_file.txt", "mtspectra_file.txt", "ptspectra_file.txt", "v2_file.txt",
               "meanmt0_midrapidity_file.txt",
               "meanpt_midrapidity_file.txt", "midrapidity_yield_file.txt", "total_multiplicity_file.txt",
               "cos2phi_file.txt","sin2phi_file.txt","cosphi_file.txt","sinphi_file.txt","sinrap.txt","cosrap.txt"]
    else:
